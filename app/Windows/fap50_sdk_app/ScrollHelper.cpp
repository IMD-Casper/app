...
const CSize& CScrollHelper::GetScrollMax() const
{
    static CSize max = (m_displaySize - m_pageSize);
    return max;
}
...
void CScrollHelper::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
    ...
    //TRACE("OnHScroll() maxScrollPos:%d newScrollPos:%d\r", maxScrollPos, newScrollPos);
...
void CScrollHelper::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
    ...
    case SB_DRAGIMAGE:
        deltaPos = nPos - m_scrollPos.cy;
        break;

    default://drag the image
        // We don't process other scrollbar messages.
        return;
    }
